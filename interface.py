...
def interface():
    ...
    frm_button.pack()
    btn_delete = tk.Button(master=frm_button, text='AC', width=5, height=2, bg='dark grey', command=delete)
    ...
    btn_equelly = tk.Button(master=frm_button, text='Вычислить', width=12, height=2, bg='orange', command=equally)
    ...
    for i in btn_list:
        ...
        if i == '-' or i =='+' or i =='/' or i =='*' in btn_list:
            tk.Button(master=frm_button, text=i, command=cmd, width=5, height=2, bg='orange').grid(row=r, column=c)
        c += 1
        if c > 3:
            c = 0
            r += 1
